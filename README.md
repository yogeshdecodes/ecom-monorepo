...
[Demo deployment](https://cra-node.herokuapp.com/): example API call from the React UI is [fetched with a relative URL](client/src/App.js#L16) that is served by an Express handler in the Node server.
...
1. Remove **create-react-app-buildpack** from the app; [heroku/nodejs buildpack](https://devcenter.heroku.com/articles/nodejs-support#activation) will be automatically activated
    ...
    ```
1. Move the root React app files (including dotfiles) into a `client/` subdirectory

    ```bash
    mkdir client
    git mv -k [!client]* client/
    mv node_modules client/
    
    # If you see "fatal: Not a git repository", then fix that error
    mv client/.git ./
    ```
    ⚠️ *Some folks have reported problems with these commands. Using the `bash` shell will probably allow them to work. Sorry if they do not work for you, know that the point is to move **everything** in the repo into the `client/` subdirectory. Except for `.git/` which should remain at the root level.* 
